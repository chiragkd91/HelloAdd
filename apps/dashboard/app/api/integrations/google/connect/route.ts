...
/**
 * Google Ads OAuth start (YouTube line items use the same Google Ads account — same OAuth).
 * If `GOOGLE_CLIENT_ID` is set, redirects to Google’s consent screen; otherwise uses the local stub.
 */
export async function GET(req: NextRequest) {
  const auth = await requireUserOrgRole(req, "MANAGER");
  if (!auth.ok) return auth.response;

  const gate = await assertAgencyOAuthGates(auth.organizationId, "GOOGLE", "google");
  if (gate) {
