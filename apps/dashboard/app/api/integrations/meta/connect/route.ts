...
/**
 * Meta OAuth start.
 * If `META_APP_ID` is set, redirects to Facebook's dialog. Otherwise uses the local stub (demo code).
 */
export async function GET(req: NextRequest) {
  const auth = await requireUserOrgRole(req, "MANAGER");
  if (!auth.ok) return auth.response;

  const gate = await assertAgencyOAuthGates(auth.organizationId, "FACEBOOK", "meta");
  if (gate) {
